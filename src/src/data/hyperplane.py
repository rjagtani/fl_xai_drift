...
class HyperplaneDataGenerator(BaseDataGenerator):
    ...
    def _generate_pool(self, mag_change: float, n_total: int) -> np.ndarray:
        ...
        # River's Hyperplane only accepts mag_change in [0, 1]; clamp so callers can use logical levels.
        mag_clamped = float(np.clip(mag_change, 0.0, 1.0))
        stream = synth.Hyperplane(
            seed=HYPERPLANE_COEFF_SEED,
            n_features=self.n_features,
            n_drift_features=self.n_drift_features if mag_clamped > 0 else 0,
            mag_change=mag_clamped,
