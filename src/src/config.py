...
class DatasetConfig:
    ...
    classification_function_post: int = 2  # Classification function after drift (concept drift)
...
class DiagnosisConfig:
    ...
    Default values match Hyperplane experiments for consistency across all datasets.
    
    Within-window calibration for Dist(FI):
    - With window_size=9, rds_window=5, n_calibration=3:
    - FI computed for 9 rounds (e.g., 242-250 if trigger at 250)
    - RDS at rounds 247, 248, 249 used for calibration (mu, sigma)
    - RDS at round 250 (trigger) checked against threshold = mu + alpha * sigma
    """
    
    window_size: int = 9  # Number of rounds before trigger to analyze
    
    # Feature importance computation
    background_size: int = 50  # K-means cluster size for background
    estimation_fraction: float = 0.1  # Fraction of validation set for estimation
    ...
    use_kmeans_background: bool = True
    ...
    def save(self, path: Optional[Path] = None):
        ...
        config_dict = {
            ...
            'diagnosis': {
                ...
                'estimation_fraction': self.diagnosis.estimation_fraction,
