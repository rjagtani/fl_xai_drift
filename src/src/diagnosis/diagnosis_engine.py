...
from .mean_fi_ph import MeanFIPHDiagnosis, MeanFIPHResult
...
class DiagnosisEngine:
    ...
    3. Running diagnosis methods (Dist(FI), Mean(FI)+PH)
    4. Saving results
    """
    ...
    def __init__(
        ...
        self.dist_fi = DistFIDiagnosis(
            ...
            n_calibration=config.diagnosis.dist_fi_n_calibration,
            alpha=config.diagnosis.dist_fi_alpha,
        )
        self.mean_fi_ph = MeanFIPHDiagnosis(
            ph_delta=config.trigger.ph_delta,
            ph_lambda=5.0,  # Fixed value for feature-level PH
        )
    ...
    def compute_fi_for_round(
        ...
        for client_id in range(n_clients):
            ...
            est_fraction = cfg.diagnosis.estimation_fraction
            X_est, y_est = self.sage_computer.create_estimation_set(
                client_data.X_val,
                client_data.y_val,
                fraction=est_fraction,
    ...
    def run_diagnosis(
        ...
        dist_results = self.dist_fi.diagnose_with_calibration(fi_matrices, diagnosis_rounds)
        
        # Run Mean(FI)+PH diagnosis
        meanph_results = self.mean_fi_ph.diagnose(fi_matrices)
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        # Save rankings and calibration info
        rankings = {}
        ...
        for name, result in results['dist_fi'].items():
            ...
            if result.rds_series is not None:
                ranking_entry['rds_series'] = result.rds_series.tolist()
                ranking_entry['rds_rounds'] = result.rds_rounds
            if result.thresholds is not None:
                ranking_entry['thresholds'] = result.thresholds.tolist()
            if result.triggered_features is not None:
                ranking_entry['triggered_features'] = result.triggered_features.tolist()
            if result.calibration_mu is not None:
                ranking_entry['calibration_mu'] = result.calibration_mu.tolist()
            if result.calibration_sigma is not None:
                ranking_entry['calibration_sigma'] = result.calibration_sigma.tolist()
            rankings[name] = ranking_entry
        
        for name, result in results['mean_fi_ph'].items():
            rankings[name] = {
                'ranking': result.feature_ranking.tolist(),
                'scores': result.ph_scores.tolist(),
            }
